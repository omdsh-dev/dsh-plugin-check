...
This plugin has been migrated to the DSH 0.1.2-alpha.3 harness and fully validated end-to-end in an isolated consumer of `local harness 0.1.2-alpha.3`:
...
- **Consumption validation**: the tarball is installed into a 0.1.2-alpha.3 consumer → `dsh --profile compat --dump-config` shows this plugin's row → the tool actually registers and executes successfully
...
The repository lives at [omdsh-dev/dsh-plugin-check](https://github.com/omdsh-dev/dsh-plugin-check) (public). Install this plugin as a standalone bundle into a profile (DSH 0.1.2-alpha.3):
