...
- **消费验证**：tarball 装入 0.1.0-rc.8 consumer → `dsh --profile compat --dump-config` 出现本插件 row → 工具真实注册与执行通过
...
仓库位于 [omdsh-dev/dsh-plugin-check](https://github.com/omdsh-dev/dsh-plugin-check)（public）。将本插件作为独立 bundle 安装到 profile（DSH 0.1.0-rc.8（npm））：
